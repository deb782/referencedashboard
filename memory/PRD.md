...
---
## PHASE 37 · Production file-download 500 hardened + diagnostics (2026-06)
- Symptom (PRODUCTION only): clicking a Performa Invoice in the Documents column opened /api/files/{id}/download and returned a raw "Internal Server Error" (500). Preview round-trip (upload→download) works 200 with the SAME code, so the code path is correct when the object exists in the same environment.
- Root cause of the RAW 500: the DEPLOYED production code still did a blind `resp.raise_for_status()` in init_storage/get_object, so any non-200 from the Emergent object-storage proxy (401 wrong key/proxy, 402 billing suspended, 403 key inactive, 404 object missing / dead session key) became an unhandled 500. The graceful handler added in a prior session was never deployed.
- Fix (server.py): introduced StorageError(status, body). init_storage() and get_object() now raise StorageError on any non-200 (get_object still force-re-inits once on 404). download_file() maps it to clear HTTP responses — 404 "no longer available, re-upload" / 503 "storage temporarily unavailable (billing/key)" / 503 generic — and LOGS the exact upstream status + body (log.error) so the true cause is visible in production logs. No more raw 500 under any failure mode.
- Verified in preview: valid download 200, missing file 404; backend boots clean, no lint errors.
- NOTE: backend/.env hardcodes EMERGENT_LLM_KEY and does NOT set INTEGRATION_PROXY_URL. load_dotenv() uses override=False so platform-injected production values win IF the platform injects them. Could not fetch production runtime logs from the agent environment (deployment_agent only did static analysis).
- ACTION FOR USER: **Redeploy**, then click a Performa Invoice again. It will now show a specific message instead of 500, and production logs will show the exact storage status. If the message is billing/key related (402/403), add balance/activate the Universal Key (Profile → Manage plan → Universal Key → Add Balance); if it says "no longer available, re-upload", the stored object is gone and the file should be re-uploaded. Report back the message/log line and I'll finish the diagnosis.

---
## PHASE 37b · Production file-download 500 — root cause PROVEN via direct prod probing (2026-06)
- Probed production directly with the real (non-expired) token from the failing link:
  - valid token + NONEXISTENT file_id → clean JSON 404 "File not found" (routing + token OK)
  - garbage token → JSON 401 "Invalid token"; no token → JSON 401 "Missing token"
  - valid token + the REAL file (file_d42b361689ac) → RAW text "Internal Server Error" (500), content-length 21.
- CONCLUSION 1: production is STILL running OLD download code. The DB record IS found (else 404), then it raw-500s at the storage-retrieval step — my committed fix would have returned JSON 502/503. My fix (commit 2b1bd92, StorageError + try/except) is committed and the working tree is clean, so the user's redeploy did NOT actually pick up the latest backend. → Needs a fresh redeploy.
- CONCLUSION 2 (the deeper cause): reproduced the mechanism in PREVIEW by inserting a file record whose object does NOT exist in storage → the Emergent object-storage proxy responds **HTTP 500** (body {"error":"Internal server error","code":"internal_server_error"}) for an unretrievable/missing object, NOT 404. Old code's blind resp.raise_for_status() turns that into the raw 500 the user sees. So the production file OBJECTS appear unretrievable (missing/lost from storage) even though their DB records survive. Real uploaded files in preview still download 200 — only missing objects 500.
- Hardening (server.py, committed): get_object raises StorageError on any non-200; download_file maps 404→404 "re-upload", 402/403→503 "billing/key", any other (incl. proxy 500)→502 "could not retrieve — may need re-upload, or storage temporarily unavailable"; logs exact upstream status+body. Verified in preview: valid 200 / unretrievable-object 502 / missing-record 404.
- ACTION FOR USER: (1) Redeploy AGAIN (latest fix is committed) and let it finish — production will then show a clear message instead of the raw 500. (2) The stored objects for the affected files look lost in production storage; the fix cannot resurrect a missing object — those PIs/POs likely need RE-UPLOADING. If a fresh upload on the live app also fails to open, it points to a storage account/key issue → contact Emergent Support (agent has no access to production storage/runtime logs).
- OPEN OFFER: build a "Replace / Re-upload file" action on each procurement request so lost PI/PO/Tax files can be re-attached in one click.

---
## PHASE 38 · Procurement: GridFS storage + flow Site→Admin→Accounts + reject re-upload + rejected-doc history + filter tabs (2026-06)
- FILE STORAGE (fixes prod "Internal Server Error"): switched procurement documents (PI/PO/Tax Invoice) to MongoDB GridFS (fs_bucket bucket_name="uploads"). save_upload() now upload_from_stream → stores gridfs_id + storage_backend="gridfs" on the db.files record. download_file GET /api/files/{id}/download serves from GridFS (open_download_stream); legacy object-storage records still attempt get_object with a clean error (StorageError → 404/502/503, never a raw 500). NOTE: files uploaded before this change live in the old failing object-storage and must be RE-UPLOADED once (user accepted this trade-off).
- FLOW CHANGE: Site Manager → Admin → Accounts. admin approve → status "approved" (goes straight to Accounts); notifies accounts + management (parallel). MANAGEMENT is optional/parallel: mgmt-action records mgmt_decision/mgmt_note and notifies admin but NEVER changes status. Startup migration advances any legacy pending_management/management_clarification → approved. Frontend: STAGES now Site/Admin/Accounts; ManagementDashboard relabelled "Approved · optional review" and queries approved/po_issued with mgmt_action_at=None.
- REJECT → RE-UPLOAD: resubmit-pi now allowed from "rejected" (also pending_clarification/pending_admin). Site Manager "Upload new PI" button shows on rejected too; ResubmitDialog now REQUIRES a fresh file. Reopens request to pending_admin.
- REJECTED-DOC HISTORY: on resubmit with a new file, the previous PI (+ admin reason, items, amount, outcome) is archived into pi_history[] (model field added). Documents column shows a conditional "<N> rejected doc(s)" link (data-testid rejected-docs-<id>) ONLY when pi_history exists → HistoryDialog (pi-history-list) lists each old rejected PI with reason + a working "View document" link. Old rejected files remain downloadable (GridFS).
- FILTER TABS: Procurement page has Active / Approved / Rejected / Cancelled / All tabs (data-testid proc-filter-tabs, proc-filter-<key>) with live counts.
